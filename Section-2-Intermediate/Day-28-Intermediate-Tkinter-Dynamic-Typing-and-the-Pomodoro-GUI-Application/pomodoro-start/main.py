...
def count_down(count):
    ...
    if count > 0:
        ...
        for _ in range(math.floor(reps / 2)):
            marks_temp += "✔ "
        check_marks.config(text=marks_temp)
...
# Creating a canvas to layer different components on top of it
canvas = Canvas(width=200, height=224, bg=YELLOW, highlightthickness=0)
center_img = PhotoImage(file="tomato.png")
canvas.create_image(100, 112, image=center_img)
...
check_marks = Label(fg=GREEN, font="bold", bg=YELLOW)
